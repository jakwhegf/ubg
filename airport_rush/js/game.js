...
var levelSpeed_arr = {planeMoveSpeed:.1, landingTimer:5, actionTimer:2}; //the total number increase for game speed
...
var playerData = {level:'', score:0, targetScore:0, landingTimer:0, actionTimer:0, planeMoveSpeed:0, first:true};
var planeData = [];
...
function buildGameButton(){
	...
	buttonLeaderboard.addEventListener("click", openLeaderboard);
        buttonSubmitScore.cursor = "pointer";
        buttonSubmitScore.addEventListener("click", openLeaderboard);
        buttonLeaderboardMenu.cursor = "pointer";
        buttonLeaderboardMenu.addEventListener("click", openLeaderboard);
        if(s_sponsor === false)
        {
...
{
    playSound('soundButton');
    if(s_isLogin === true || curPage === 'main')
    {
...
{
    console.log('submitScore ' + s_iScore)
    if(s_isLogin === true)
    {
     ID.GameAPI.Leaderboards.save({'table':'Leaderboard','points':s_iScore});   
    }
}
     
function func_y8Logo()
{
    if(s_sponsor === false)
    {
...
function startGame(){
	...
	for(n=0;n<terminals_arr[terminalNum].runway.length;n++){
		landingTimer_arr.push({timer:null});
		startLandingTimer(n);
	}
}
...
function createPlane(curRunwayNum){
	...
	planeData.push({type:planeTypeNum, plane:$.planes[planeNum], shadow:$.planes['s'+planeNum], mode:0, value:{x:0, y:0, oldX:0, oldY:0, lastX:0, lastY:0}, level:{val:0}, interval:null, timer:{count:0, store:0, max:0, skip:false}, runwayNum:curRunwayNum, gateNum:gateNum});
...
function animatePlane(num){
	...
	if(curPlaneMode == 0){
		...
		TweenMax.to(planeData[num].level, timeSpeed/2, {val:0, ease:easeMode, overwrite:true});
	}else if(curPlaneMode == 7){
		planeData[num].level.val = 0;
		TweenMax.to(planeData[num].level, timeSpeed/2, {delay:timeSpeed/2, val:100, ease:easeMode, overwrite:true});
	}
	
	TweenMax.to(planeData[num].value, timeSpeed, {level:0, bezier:{curviness:0, autoRotate:true, values:runwayPath}, ease:easeMode, overwrite:true, onComplete:animatePlaneComplete, onCompleteParams:[num], onUpdate:updatePlaneMove, onUpdateParams:[num]});
}
...
function updatePlaneDepth(){
	...
	for(n=0; n<planeData.length; n++){
		if(planeData[n].plane != null){
			planeDepth_arr.push({id:n, y:planeData[n].shadow.y + planeData[n].level.val});
		}
	}
